...
export function lzwDecode(t: string): string {
    ...
    for (let r = 1; r < t.length; r++) {
        ...
        let l: string
        l = a < 256 ? t.substr(r, 1) : e[a] ? e[a] : n + i
        o.push(l)
        i = l.substr(0, 1)
        e[s] = n + i
        s++
...
function n(t: string): number[] {
    const i = encodeURIComponent(t).split('')
    const n: number[] = []
    for (let t = 0; t < i.length; t++) {
        if (i[t] === '%') {
            n.push((e.indexOf(i[t + 1].toLowerCase()) << 4) | e.indexOf(i[t + 2].toLowerCase()))
            t += 2
        } else {
            n.push(i[t].charCodeAt(0))
        }
    }
    return n
}
function i(t: number[], e: number[]): number[] {
    let i: number
    let n: number
    let s: number
    const o: number[] = []
    ...
    for (i = 0, n = 0; i < 256; i++) {
        n = (n + o[i] + t[i % t.length]) % 256
        s = o[i]
        o[i] = o[n]
        o[n] = s
    }
    for (let t = 0, i = 0, n = 0; t < e.length; t++) {
        i = (i + 1) % 256
        n = (n + o[i]) % 256
        s = o[i]
        o[i] = o[n]
        o[n] = s
        r.push(e[t] ^ o[(o[i] + o[n]) % 256])
    }
    return r
}
export function decrypt(t: string, s: string): string {
    return (function (t: number[]) {
        let i = ''
        for (let n = 0; n < t.length; n++) i += `%${e.charAt((t[n] >> 4) & 15)}${e.charAt(15 & t[n])}`
        return decodeURIComponent(i)
    }(i(n(t), (function (t: string): number[] {
