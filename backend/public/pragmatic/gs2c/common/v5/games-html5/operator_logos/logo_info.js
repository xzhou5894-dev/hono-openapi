...
UHTPatch({
	...
	apply: function () {
		if (["vswayshexhaus", "vswayshexhaus_cv99", "vswaysultrcoin", "vswaysultrcoin_cv89"].indexOf(window["UHT_GAME_CONFIG"]["GAME_SYMBOL"]) != -1) {
	...
	apply: function () {
		...
		createjs.Sound.setVolume = function (a) {
			...
			if (a = Math.max(0, Math.min(1, a)), /**/ a *= 0.8, /**/
				createjs.Sound._masterVolume = a, !this.activePlugin || !this.activePlugin.setVolume || !this.activePlugin.setVolume(a))
	...
	apply: function () {
		var templateJS = {
			...
			"winInfo": {
				"totalWinCount": 0,
				"biggestWinAmount": "0",
				"biggestWinDate": "01/01/1001",
				"biggestWinnerID": "**00",
				"latestWinAmount": "0",
				"latestWinDate": "01/01/1001",
				"latestWinnerID": "YOU"
			},
			"name": "AAA",
			"amount": "0",
			...
			"nextSeedAmountUSD": "0",
			"contributePercent": "0",
			"babyContributeInitialPercent": "0",
			"babyContributePercent": "0",
			"jackpotBabyAmount": "0",
			"jackpotBabyAmountUSD": "0",
	...
	apply: function () {
		...
		if (!isMini) {
			if (window["FreeSpinsPurchaseManager"])
				window["FreeSpinsPurchaseManager"].prototype.EnoughMoney = function () {
					return true;
				}

			if (window["FeaturePurchaseManager"])
				window["FeaturePurchaseManager"].prototype.EnoughMoney = function () {
					return true;
				}

			if (window["FeaturePurchaseV2"])
				window["FeaturePurchaseV2"].prototype.EnoughMoney = function () {
					return true;
				}

			if (window["FeaturePurchaseManager_GRM"])
				window["FeaturePurchaseManager_GRM"].prototype.EnoughMoney = function () {
					return true;
				}

			if (window["FeaturePurchaseManager_BK"])
				window["FeaturePurchaseManager_BK"].prototype.EnoughMoney = function () {
					return true;
				}
		}
	...
	apply: function () {
		...
		var OnXTGameInit = function () {
			...
			StageResult.prototype.CanSpin = function () {
				...
				if (showBetWarning && !IsFeatureBought() && !CoinManager.isStrictMode && (window["inHyperplay"] != true)) {
...
UHTPatch({
	...
	apply: function () {
		var canOpenHyperplay = IsRequired("HYPERPLAY");
		if (canOpenHyperplay) {
			var reqNOK = ["SB", "SS", "NOTS", "SR", "NOAP", "NOFP", "LBLFS", "LBLG"];
			for (var i in reqNOK) {
		...
		var withHyperplay = {
			...
			"vs10jokerhot": {
				...
				"v": 1,
				"clones": {
					"woVisual": ["vs10bblpop", "vs10bblwild", "vs10emotiwins", "vs10firestrike88", "vs10firestrikedf", "vs10goalstrike", "vs10jokerhot", "vs10jokerhotlim", "vs10sfrenzy", "vs10wrldsport"]
				}
			},
			"vs5jjwild": {
				"sett": 1,
			...
			"vs20doghouse": {
				...
				"v": 1,
				"clones": {
					"woVisual": ["vs20amuleteg", "vs20birdhouse", "vs20dhdice", "vs20dhnovi", "vs20doghousejp", "vs20havefes", "vs20roohouse", "vs20tweethouse"]
				}
			},
			"vs20sugarrush": {
				"sett": 1,
...
UHTPatch({
	...
	apply: function () {
		var ignoreGame = ["vs20terrorv", "vs5drmystery", "vs432congocash", "vs20gorilla", "vs25champ", "vs3train", "vs40wanderw", "vs10snakeladd", "vs20rainbowg", "vs20ltng", "vs75bronco",
			...
			"vs20rainbowg_cv79", "vs20ltng_cv88", "vs75bronco_cv47", "vs5drmysterygr_cv64", "vs20gorillagr_cv58", "vs25champgr_cv61", "vs75broncogr_cv58"
		].indexOf(UHT_CONFIG.SYMBOL) > -1;
...
var CasePersistentTextReplace = function (inout /*._text*/, replace) {
	...
	var s = replace.s ? "\\b" : "";
	//CamelCase
	var src = replace.src.split('');
	src[0] = src[0].toUpperCase();
	src = src.join('');
	var dst = replace.dst.split('');
	...
	apply: function () {
		var TReplacements = {
			...
			pt___BR: [
				...
				{
					src: "têm um multiplicador da aposta total atribuído.",
					dst: "têm atribuído um multiplicador da aposta total."
				},

				{
					src: "or mais símbolos",
					dst: "ou mais símbolos",
					p: true,
					s: true
				},
				{
					src: "rolos e pode surgir",
					dst: "rolos e podem surgir",
					p: true,
					s: true
				},
				{
				...
				{
					src: "BONUS presentes",
					dst: "BONUS estão presentes"
				},
				{
					src: "SCATTER presentes",
					dst: "SCATTER estão presentes"
				},
				{
					src: "do  símbolo",
					dst: "do símbolo",
				...
				{
					src: "Recolha de símbolo",
					dst: "Recolha do símbolo",
					p: true,
					s: true
				},
				{
					src: "DINHEIRO COMBOIO",
					dst: "COMBOIO DO DINHEIRO",
					p: true,
					s: true
				},
				...
				{
					src: "numa respin",
					dst: "numa nova rodada",
					p: true,
					s: true
				},
				{
					src: "para a direita e da direita a esquerda",
					dst: "para a direita e da direita para a esquerda",
					p: true,
					s: true
				},
				{
					src: "ativar o PRÉMIOS",
					dst: "ativar os PRÊMIOS",
					p: true,
					s: true
				},
				{
					src: "numa posição aleatória adicionado ao ecrã.",
					dst: "adicionado numa posição aleatória na tela.",
					p: true
				},
				{
					src: "a característica RESPIN é accionada e 5 respinas são atribuídas.",
					dst: "o extra RESPIN é acionado e são atribuídas 5 novas rodadas.",
					p: true
				},
				{
					src: "Durante a CARACTERÍSTICA DE RESPINA todos os símbolos SCATTER permanecem no ecrã até ao final da rodada e o número de símbolos em cada bobina permanece o mesmo que a rotação que desencadeia a característica.",
					dst: "Durante o EXTRA DE RESPIN todos os símbolos SCATTER permanecem na tela até ao final da rodada e o número de símbolos em cada rolo permanece o mesmo que a rodada que desencadeou o extra."
				},
				{
					src: "O jogo paga as combinações vencedoras e as rodadas vencedoras normalmente entre cada respina. A rodada termina quando todas as 5 respinas ou um símbolo SCATTER estão presentes em cada carretel.",
					dst: "O jogo paga as combinações e as rodadas vencedoras, geralmente, entre cada rodada. A rodada termina quando todas as 5 rodadas ou um símbolo SCATTER estejam presentes em cada carretel."
				},
				{
					src: "Se o ecrã estiver preenchido",
					dst: "Se a tela estiver preenchida",
					p: true
				},
				{
					src: "O prémio de Rodadas Grátis e Nova Rodada é atribuído ao jogador após a conclusão da rodada.",
					dst: "O prêmio em Rodadas Grátis e Nova Rodada é atribuído ao jogador no final do ciclo completo."
				},
				{
					src: "um novo respin é acionado. Se mais símbolos WILD forem atingidos como resultado do respin, expandem-se e acionam mais uma respin. O ciclo de respin termina quando",
					dst: "uma nova rodada é acionada. Se mais símbolos WILD forem atingidos como resultados da nova rodada, expandem-se e acionam mais uma rodada. O ciclo de rodadas termina quando"
				},
				{
					src: "Pagar",
					dst: "Pague",
				...
				{
					src: "Os símbolos dão prémio",
					dst: "Os símbolos são prêmio",
					p: true
				},
				{
					src: "Após a 4 reativação",
					dst: "Após a 4ª reativação",
					p: true
				},
				{
					src: "Sempre o BARCO",
					dst: "Sempre que o BARCO",
					p: true
				},
				{
					src: "A funcionalidade",
					dst: "O extra",
					p: true
				},
				{
				...
				{
					src: "rodadas grátis da arriscar",
					dst: "rodadas grátis da aposta",
					p: true,
					s: true
				},
				{
					src: "Antes do início da rodada jogador",
					dst: "Antes do início da rodada, o jogador",
					p: true,
					s: true
				},
				{
					src: "de linha de pagamento",
					dst: "nas linhas de pagamento",
					p: true,
					s: true
				},
				{
					src: "nas linha de pagamento",
					dst: "nas linhas de pagamento",
					p: true,
					s: true
				},
				...
				{
					src: "sequência de tumbling",
					dst: "sequência de quedas",
					p: true,
					s: true
				},
				{
				...
				{
					src: "todas posições",
					dst: "todas as posições",
					p: true,
					s: true
				},
			],
			pl: [{
				src: "DARMOWE OBROTY BONUSU",
				dst: "DARMOWE SPINY",
				p: true,
				s: true
			},],
		...
		if (UHT_CONFIG.SYMBOL != "vs7monkeys_dev" && !IsRequired("VAPP")) {
			SystemMessageShowVerifySteps = function () {
				...
				if (window["UHT_GAME_CONFIG"]["LANGUAGE"] == "ko")
					CustomMsgManager.HandleResponse(null, {
						custom_msg: JSON.stringify([{
							title: "",
		...
		VerifyGameAuthenticityManager.prototype.OpenIframe = function () {
			if (!initialized) {
				...
				if (document.getElementById("closePort"))
					document.getElementById("closePort").addEventListener("click", function () {
						Globals.GamePaused = false;
					}, false);
	...
	apply: function () {
		if (IsRequired("SOCE") && (UHT_CONFIG.LANGUAGE == "en")) {
			...
			SOCE_replacements.splice(0, 0, {
				src: "to place this bet, you will need to visit the cashier and add funds to your account",
				dst: "Insufficient coins to place the spin"
			}, {
				src: "insufficient funds to place this bet.",
				dst: " "
			}, {
				src: "please fund your account or lower the bet level.",
				dst: "Insufficient coins to place the spin"
			});
			SOCE_replacements.push({
...
function PatchCurrency() {
	...
	CurrencyPatch.prototype.InitCurrencyInfo = function () {
		...
		this.languageFormats["tr_dsep"] = ",";
		this.languageFormats["tr_dnum"] = "2";
		this.languageFormats["tr_gsep"] = ".";
		this.languageFormats["tr_gnum"] = "3";
		this.languageFormats["tr_symp"] = "3";

		this.languageFormats["fi_symp"] = "1";

		if (IsRequired("ES-419")) {
			this.languageFormats["es_dsep"] = ".";
			this.languageFormats["es_gsep"] = ",";
			this.languageFormats["es_symp"] = "0";
		}
...
function PatchDisableTurboSpin() {
	...
	var OnXTGameInit = function () {
		if (Globals.isMobile) {
			...
			var autoplay = window["AutoplayControllerMobile"] ? globalRuntime.sceneRoots[1].GetComponentsInChildren(AutoplayControllerMobile, true) : [];
			for (var i = 0; i < autoplay.length; ++i) {a
				var turboSpin = autoplay[i].transform.Find("Content/Checkboxes/TurboSpin");
				if (turboSpin != null)
					turboSpin.gameObject.SetActive(false);
			}
